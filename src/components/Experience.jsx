...
const BubbleGlow = ({
  ...
  return (
    <motion.div
      initial={{ boxShadow: `0 0 15px ${borderWidth * 2}px rgba(255, 215, 0, 0.4)`, scale: 1 }}
      ...
      className={`
        flex items-center justify-center rounded-full bg-black z-20
        relative mx-auto mb-6 md:mb-0
        md:absolute md:top-[15%] md:translate-y-[-50%]
        ${position === "left" ? "md:left-[-40px]" : "md:right-[-40px] md:left-auto"}
      `}
      style={{
        width: size,
        height: size,
...
const TimelineCard = ({
  ...
  return (
    <motion.div
      className="relative flex flex-col md:flex-row items-center justify-center mb-10 w-full"
    ...
    >
      ...
      <motion.div
        ...
        className={`mx-2 md:mx-10 p-0 md:p-5 flex-1 ${width} w-full`}
      >
        <CustomTilt
          options={{
            max: 15,
        ...
        >
          <div
            className="rounded-xl border-2 bg-gradient-to-br from-neutral-900 via-neutral-900/90 to-neutral-900/60 shadow-lg backdrop-blur-xl px-4 py-6 md:px-6 md:py-6 w-full min-w-0 md:min-w-[420px] transition-all duration-300 hover:shadow-[0_0_20px_5px_#FFD700] relative overflow-hidden"
          ...
          >
            ...
            {/* Animated Scan Line - disable in Performance Mode */}
            {!performanceMode && (
              <motion.div
...
const ExperienceSection = () => {
  ...
  return (
    ...
    >
      ...
      {/* Enhanced Floating Graphics - disable in Performance Mode */}
      {!performanceMode && <FloatingExperienceGraphics />}
      
      <div className="relative z-10 max-w-6xl mx-auto">
        <h2 className="text-4xl md:text-6xl font-extrabold text-center mb-6 bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 via-orange-400 to-yellow-300 drop-shadow-lg px-4">
          Project Journey
        </h2>
        
        <p className="text-gray-200 text-base md:text-lg text-center mb-10 max-w-3xl mx-auto px-4">
