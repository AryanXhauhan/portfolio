...
const Hero = () => {
  ...
  return (
    <section className="relative w-full h-screen bg-black overflow-hidden flex items-center justify-center">
      ...
      {/* BACKGROUND GRAPHICS: Conditionally rendered under Performance Mode */}
      {!performanceMode ? (
      ...
      {/* Particles network: disabled entirely in Performance Mode */}
      {!performanceMode && (
        <Particles
          options={{
      ...
      {/* Minimal Sparkle Elements: Render only in Visual Mode */}
      {!performanceMode && sparkleShapes.map(({ top, left, right, size, delay }, idx) => (
        <motion.div
          key={`sparkle-${idx}`}
      ...
      {/* Floating Circles: Render only in Visual Mode */}
      {!performanceMode && floatingCircles.map(({ top, left, right, size, delay }, idx) => (
        <motion.div
          key={`circle-${idx}`}
          initial={{ opacity: 0 }}
          animate={{
      ...
      {/* Code Snippets: Static layout in Eco Mode, Animated in Visual Mode */}
      {codeSnippets.map((snippet, i) => {
        const Comp = performanceMode ? "div" : motion.div;
        const animationProps = performanceMode
          ? {}
          : {
              initial: { opacity: 0, y: 50 },
              animate: { opacity: 0.95, y: [0, 8, 0] },
              transition: {
        ...
        return (
          <Comp
            key={i}
            {...animationProps}
            className="hidden md:block absolute bg-[#18181b] border border-[#423b51] rounded-xl p-5 w-[273px] shadow-2xl font-mono text-[#8b5cf6]"
            style={{
      ...
      {/* Floating outlined rectangles: Render only in Visual Mode */}
      {!performanceMode && floatingRects.map(({ top, left, right, rotate, delay }, i) => (
        <motion.div
          key={`rect-${i}`}
          initial={{ opacity: 0 }}
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center px-5">
        <motion.h1
          ...
          className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-yellow-300 via-purple-400 to-yellow-600 leading-tight"
        >
          Aryan Chauhan
        </motion.h1>
      ...
      <motion.div
        animate={performanceMode ? {} : { y: [0, -10, 0] }}
      ...
      >
        <a href="#about" className="relative">
          <div className="w-6 h-10 border-2 border-yellow-400 rounded-full flex justify-center items-center">
            <motion.div
              animate={performanceMode ? {} : { y: [0, 11, 0] }}
