...
const HighPerformanceGlobe = () => {
  return (
    <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
      {/* Luxury Ambient Glows */}
      <div className="absolute w-[350px] h-[350px] rounded-full bg-gradient-to-tr from-yellow-500/10 via-orange-500/5 to-transparent blur-3xl" />
      <div className="absolute w-[200px] h-[200px] rounded-full bg-purple-500/5 blur-2xl animate-pulse" />
      
      {/* Spinning Outer Ring */}
      <div className="absolute w-[320px] h-[320px] rounded-full border border-dashed border-yellow-500/20 animate-[spin_40s_linear_infinite]" />
      
      {/* Spinning Inner Ring */}
      <div className="absolute w-[260px] h-[260px] rounded-full border border-dashed border-orange-500/10 animate-[spin_20s_linear_infinite_reverse]" />

      {/* SVG Wireframe Earth */}
      <svg className="w-[300px] h-[300px] relative z-10 text-yellow-500/30" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="0.5">
        {/* Outer Sphere Border */}
        <circle cx="50" cy="50" r="40" strokeWidth="1" className="stroke-yellow-500/40" />
        
        {/* Latitudes */}
        <ellipse cx="50" cy="50" rx="40" ry="12" />
        <ellipse cx="50" cy="50" rx="40" ry="24" />
        <line x1="10" y1="50" x2="90" y2="50" strokeWidth="0.8" className="stroke-yellow-500/40" />
        
        {/* Longitudes */}
        <ellipse cx="50" cy="50" rx="12" ry="40" />
        <ellipse cx="50" cy="50" rx="24" ry="40" />
        <line x1="50" y1="10" x2="50" y2="90" strokeWidth="0.8" className="stroke-yellow-500/40" />

        {/* Floating Connection Points / Tech Nodes */}
        <circle cx="26" cy="30" r="1.5" className="fill-yellow-400 stroke-yellow-300 animate-ping" strokeWidth="0.5" />
        <circle cx="26" cy="30" r="1" className="fill-yellow-400" />
        
        <circle cx="74" cy="70" r="1.5" className="fill-orange-400 stroke-orange-300 animate-pulse" strokeWidth="0.5" />
        <circle cx="74" cy="70" r="1" className="fill-orange-400" />

        <circle cx="50" cy="20" r="1" className="fill-yellow-300" />
        <circle cx="68" cy="38" r="1" className="fill-yellow-300" />
        <circle cx="32" cy="62" r="1" className="fill-orange-300" />
      </svg>
      
      {/* status/loading overlay indicator */}
      <div className="absolute bottom-4 flex flex-col items-center gap-1 z-20">
        <span className="text-[10px] text-gray-500 uppercase tracking-widest font-extrabold">Active Radar Feeder</span>
        <span className="text-[9px] text-green-400 font-mono flex items-center gap-1.5 bg-green-500/5 px-2 py-0.5 rounded-full border border-green-500/20">
...
// Premium Floating Contact Graphics - Optimized & Memoized
const FloatingContactGraphics = React.memo(() => {
  return (
    <div className="absolute inset-0 pointer-events-none z-0">
      {/* Animated Wave Lines */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-15" style={{ willChange: "transform, opacity" }}>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-10" style={{ willChange: "opacity" }}>
        {Array(5).fill(0).map((_, i) => {
        ...
        <motion.div
          ...
          style={{
            ...
            background: `radial-gradient(circle, ${
              i === 0 ? 'rgba(251, 191, 36, 0.04)' :
              i === 1 ? 'rgba(251, 146, 60, 0.04)' :
              'rgba(234, 179, 8, 0.04)'
            } 0%, transparent 70%)`,
            top: `${20 + i * 25}%`,
            left: `${15 + i * 30}%`,
...
const Contact = () => {
  ...
  return (
    ...
    >
      ...
      {/* Ultra Premium Floating Graphics - disable in Performance Mode */}
      {!performanceMode && <FloatingContactGraphics />}
      ...
      <div className="relative z-10 max-w-7xl w-full mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 items-start mb-16">
          ...
          >
            ...
            {/* Corner Glow Effects (Optimized) */}
            <div
              className="absolute top-0 left-0 w-32 h-32 rounded-full opacity-20 pointer-events-none"
            ...
            >
              ...
              >
                ...
                {/* Golden Reflection Glow (Optimized) */}
                {!loading && (
                  <div
                    className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3/4 h-8 rounded-full pointer-events-none opacity-50"
                    style={{
          ...
          {/* Earth Canvas / Radar Globe Fallback */}
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={inView ? { opacity: 1, x: 0 } : { opacity: 0, x: 50 }}
            transition={{ duration: 0.6, delay: 0.4 }}
            className="xl:h-[700px] md:h-[550px] h-[450px] w-full"
          >
            {performanceMode ? (
              <HighPerformanceGlobe />
            ) : (
              <React.Suspense fallback={<HighPerformanceGlobe />}>
                <LazyEarthCanvas />
              </React.Suspense>
            )}
          </motion.div>
